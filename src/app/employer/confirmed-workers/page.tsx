...
export default async function EmployerConfirmedWorkersPage() {
  ...
  const confirmed = await prisma.jobOfferRecipient.findMany({ where: { offer: { tenantId: context.tenant.id }, status: 'CONFIRMED' }, include: { partTimer: true, offer: { include: { job: true } } }, orderBy: { confirmedAt: 'desc' }, take: 100 });
