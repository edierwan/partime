...
export default async function EmployerResponsesPage() {
  ...
  const responses = await prisma.jobOfferRecipient.findMany({ where: { offer: { tenantId: context.tenant.id }, status: { in: ['INTERESTED', 'NOT_INTERESTED', 'NO_RESPONSE'] } }, include: { partTimer: true, offer: { include: { job: true } } }, orderBy: { updatedAt: 'desc' }, take: 100 });
